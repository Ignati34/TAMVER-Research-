...
/**
 * v0.6.1 collision-safe layout.
 * Multi-node stages are distributed vertically (scenario/formula) or horizontally
 * (evidence/claim) instead of crowding several wide labels on the same row.
 */
function layoutPoint(stage:ImpactStage,index:number,count:number):LayoutPoint{
  ...
  switch(stage){
    case "SCENARIO_NODE": {
      ...
      return {x:16,y:63+(index-mid)*gap,align:"left"};
    }
    case "FORMULA": {
      const gap=Math.min(12,28/Math.max(1,count-1));
      return {x:82,y:64+(index-mid)*gap,align:"left"};
    }
    case "METRIC": {
      const gap=Math.min(11,22/Math.max(1,count-1));
      return {x:18,y:28+(index-mid)*gap,align:"left"};
    }
    case "DECISION_GATE": {
      const gap=Math.min(11,22/Math.max(1,count-1));
      return {x:82,y:29+(index-mid)*gap,align:"left"};
    }
    case "REPORT_CLAIM": {
      const spread=Math.min(23,46/Math.max(1,count-1));
      return {x:50+(index-mid)*spread,y:10,align:"center"};
    }
    case "EVIDENCE_ITEM":
    default: {
      const spread=Math.min(24,48/Math.max(1,count-1));
      return {x:50+(index-mid)*spread,y:89,align:"center"};
    }
  }
}
...
export default function EvidenceImpactCrystal({evidenceId="ev-supplier-single-point"}:{evidenceId?:string}){
  ...
  useEffect(()=>{
    ...
    const camera=new THREE.PerspectiveCamera(34,el.clientWidth/el.clientHeight,.1,100);camera.position.set(0,-.02,9.45);
    ...
    const group=new THREE.Group();group.scale.setScalar(1.72);group.position.y=.18;scene.add(group);
    const geo=geometry();
    ...
    const haloGeo=new THREE.EdgesGeometry(new THREE.IcosahedronGeometry(3.55,1),7);
    ...
    let raf=0,mx=0,my=0;
    const move=(e:PointerEvent)=>{const r=el.getBoundingClientRect();mx=(e.clientX-r.left)/r.width-.5;my=(e.clientY-r.top)/r.height-.5};el.addEventListener("pointermove",move);
    ...
    const tick=()=>{
      raf=requestAnimationFrame(tick);const now=performance.now()/1000;const age=now-pulseStart;const live=!reducedMotion&&age>=0&&age<pulseDuration;const decay=live?Math.pow(1-age/pulseDuration,.62):0;
      pulseUniforms.uTime.value=live?age:-99;pulseUniforms.uStrength.value=decay*previewStrength;signalLight.intensity=live?12*decay*previewStrength:0;mat.emissiveIntensity=.08+(live?1.0*decay*previewStrength:(reducedMotion?0:.035*Math.sin(now*.9)));
      if(!reducedMotion){group.rotation.y+=.0012;group.rotation.x+=(my*.07-group.rotation.x)*.018;group.rotation.z+=(-mx*.045-group.rotation.z)*.018;halo.rotation.y-=.00055}
      renderer.render(scene,camera);
    };tick();

    return()=>{
      cancelAnimationFrame(raf);window.removeEventListener("resize",resize);el.removeEventListener("pointermove",move);
      geo.dispose();edgeGeo.dispose();edgeMat.dispose();pulseMat.dispose();innerMat.dispose();haloGeo.dispose();haloMat.dispose();mat.dispose();renderer.dispose();if(el.contains(renderer.domElement))el.removeChild(renderer.domElement);
    };
  },[]);
